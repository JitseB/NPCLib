package com.bnstra.npclib.nms.v1_18_R1;

import com.bnstra.npclib.nms.v1_18_R1.packets.*;
 ...
 */
public class NPC_v1_18_R1 extends NPCBase {
    ...
    public NPC_v1_18_R1(NPCLib instance, List<String> lines) {
        super(instance, lines, MinecraftVersion.V1_18_R1);
    }
    ...
    public void sendShowPackets(Player player) {
        ...
        if (hasTeamRegistered.add(player.getUniqueId()))
            Arrays.stream(packetPlayOutScoreboardTeam).forEach(playerConnection::a);
        playerConnection.a(packetPlayOutPlayerInfoAdd);
        playerConnection.a(packetPlayOutNamedEntitySpawn);
        playerConnection.a(packetPlayOutEntityHeadRotation);
        ...
        playerInfoRemoveTimers.put(player.getUniqueId(), Bukkit.getScheduler().runTaskLater(instance.getPlugin(), () -> {
            if (isShown(player)) {
                playerConnection.a(packetPlayOutPlayerInfoRemove);
            }
    ...
    public void sendHeadRotationPackets(Location location) {
        for (UUID shownUuid : super.getShown()) {
            ...
            if (player != null && isShown(player)) {
                ...
                connection.a(new PacketPlayOutEntity.PacketPlayOutEntityLook(entityId, (byte) ((yaw % 360.) * 256 / 360), (byte) ((pitch % 360.) * 256 / 360), false));
                connection.a(new PacketPlayOutEntityHeadRotationWrapper().create(npcLocation, entityId));
            }
        }
    }
}
